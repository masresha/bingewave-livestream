...
(function (angular) {
	angular
		...
		.controller('ContentHomeCtrl', ['$scope', '$location', 'SocialDataStore', 'ReqService',
			'ApService', 'Modals', 'Buildfire', 'EVENTS', '$timeout', 'SocialItems', 'Util',
			function ($scope, $location, SocialDataStore, ReqService, ApService, Modals,
				Buildfire, EVENTS, $timeout, SocialItems, Util) {
				...
				buildfire.datastore.get("bingeWaveConfig", (err, result) => {
					if (err) return console.error(
						"Error while retrieving your data", err);
					// console.log("bingeWaveConfig", result.data);
					ContentHome.organizer_id = result.data.organizer_id;
					ContentHome.auth_token = result.data.auth_token;
					ContentHome.template_id = result.data.template_id;
                    ContentHome.name = result.data.name;
                    ContentHome.description = result.data.description;
				});

				ContentHome.saveBingeWaveConfig = function () {
					ContentHome.data.content.organizer_id = ContentHome.organizer_id;
					ContentHome.data.content.auth_token = ContentHome.auth_token;
					...
					buildfire.datastore.save(
						...
						(err, result) => {
							if (err) return console.error(
								"Error while saving your data", err);
				...
				$scope.createOrganizer = function () {
					ContentHome.info.name = ContentHome.name;
					ContentHome.info.description = ContentHome.description;
					ContentHome.info.domain = ContentHome.name.replace(/ /g, '');;
					ContentHome.info.type = "13";
					console.log(ContentHome.info);

					ApService.create_new_organizer(ContentHome.info, function (
						result) {
						...
						ApService.get_distributor_token(credentials, function (
							result) {
							console.log(result.data.data.auth_token);
							console.log(result.data.data.organizer.id);
							ContentHome.organizer_id = result.data.data.organizer.id;
							ContentHome.auth_token = result.data.data.auth_token;
							ContentHome.template_id ="16388b97-18e1-4059-893a-6f062fdd06a7";
                            ContentHome.data.content.organizer_id = result.data.data.organizer.id;
							ContentHome.data.content.auth_token = result.data.data.auth_token;
							ContentHome.data.content.template_id = "16388b97-18e1-4059-893a-6f062fdd06a7";
                            ContentHome.data.content.name = ContentHome.name;
                            ContentHome.data.content.description = ContentHome.description;
				...
				$scope.setupImageList = function (post) {
					if (post.imageUrl) {
						...
						setTimeout(function () {
							let imageList = document.getElementById(post
								.imageListId);
							if (imageList) {
				...
				var init = function () {
							...
							if (Object.keys(data.screenOne).length <= 6) {
								Object.keys(data.screenOne).forEach((key) => {
									if (data.screenOne[key].value) {
										stringsConfig.screenOne.labels[
												key].value = data
											.screenOne[key].value;
									}
								});
								buildfire.datastore.save({
									screenOne: stringsConfig.screenOne
										.labels
								}, "languages", (err, data) => {
									console.log(data)
								});
					...
					Buildfire.getContext(function (err, context) {
						...
						Buildfire.datastore.get('Social', function (err, data) {
							...
							if (data && data.data && data.data
								...
								if (ContentHome.appSettings.appSettings
									.pinnedPost) {
									ContentHome.descriptionWYSIWYG =
					...
					ContentHome.setWYSIWYG = function () {
						if (ContentHome.appSettings) {
							ContentHome.appSettings.appSettings.pinnedPost =
								ContentHome.descriptionWYSIWYG
						} else {
							ContentHome.appSettings = {
								appSettings: {
						...
						Buildfire.datastore.save(ContentHome.appSettings, 'Social',
							function (err, result) {
								if (err) return console.log(err)
							})
					}
				...
				ContentHome.getUserName = function (userId) {
					...
					ContentHome.usersData.some(function (userData) {
						if (userData && userData.userObject && userData
					...
					ContentHome.usersData.some(function (userData) {
						if (userData && userData.userObject && userData
							...
							userImageUrl = userData.userObject.imageUrl ?
								Buildfire.imageLib.cropImage(userData.userObject
									.imageUrl, {
										width: 40,
										height: 40
									}) : '';
						...
						(err, isConfirmed) => {
							...
							if (isConfirmed) {
								...
								var success = function (response) {
									console.log('inside success of delete post',
										response);
									if (response) {
										Buildfire.messaging
											.sendMessageToWidget({
										...
										ContentHome.posts = ContentHome.posts
											.filter(function (el) {
												return el.id != postId;
											});
									}
								...
								var postToDelete = ContentHome.posts.find(element =>
									element.id === postId);
								if (postToDelete) Posts.deletePost({
								...
								SocialDataStore.deletePost(postId, ContentHome
									.socialAppId, datastoreWriteKey).then(
									success, error);
							}
						}
					);
					...
					buildfire.dialog.confirm({
						...
						(err, isConfirmed) => {
							...
							if (isConfirmed) {
								...
								var success = function (response) {
									console.log(
										'inside success of delete comment',
										response);
									if (response) {
										Buildfire.messaging
											.sendMessageToWidget({
								...
								// Deleting post having id as postId
								SocialDataStore.deleteComment(post.id, commentId)
									.then(success, error);
							}
						}
					);
					...
					buildfire.dialog.confirm({
						...
						(err, isConfirmed) => {
							...
							if (isConfirmed) {
								...
								var error = function (err) {
									console.log("Error while banning a user ",
										err);
								};
				...
				ContentHome.loadMoreComments = function (thread, viewComment) {
					...
					var newUserIds = [];
					initialCommentsLength = (thread.comments && thread.comments
						.length) || null;
					if (viewComment && viewComment == 'viewComment' && thread.comments
						.length > 0)
						thread.viewComments = thread.viewComments ? false : true;

					if (thread.commentsCount > 0 && thread.commentsCount !=
						initialCommentsLength) {
						SocialDataStore.getCommentsOfAPost({
							threadId: thread._id,
							lastCommentId: thread.comments && !viewComment ?
							...
							function (data) {
								console.log(
									'Success in Content get Load more Comments---------',
									data);
								if (data && data.data && data.data.result) {
									thread.uniqueLinksOfComments = thread
										.uniqueLinksOfComments || [];
									thread.comments = thread.comments && !
										viewComment ? thread.comments.concat(data
											.data.result) : data.data.result;
									thread.moreComments = thread.comments && thread
										.comments.length < thread.commentsCount ?
										false : true;
									thread.comments.forEach(function (commentData) {
										if (thread.uniqueLinksOfComments
											.indexOf(commentData.threadId +
												"cmt" + commentData._id) ==
											-1) {
											thread.uniqueLinksOfComments
												.push(commentData.threadId +
													"cmt" + commentData._id
												);
											newUniqueLinksOfComments.push(
												commentData.threadId +
												"cmt" + commentData._id);
										}

										if (userIds.indexOf(commentData
												.userId) == -1) {
											userIds.push(commentData
												.userId);
											newUserIds.push(commentData
												.userId);
										}
									});
				...
				ContentHome.exportMainWallPosts = function (mainCallback) {
					...
					function parseToCSV() {
						...
						allPosts.forEach(function (threadData) {
							var _data = [];
							_data.push('post');
							_data.push(moment(threadData.data.createdOn).format(
								'DD/MM/YYYY hh:mm a'));
							_data.push(threadData.data.userDetails.displayName);
							_data.push(ContentHome.util.injectAnchors(threadData
								.data.text));
							lineArray.push(_data);
							if (threadData.data.comments.length > 0) {
								threadData.data.comments.forEach(function (
									commentData) {
									var _data = [];
									_data.push('comment');
									_data.push(moment(commentData
										.createdOn).format(
										'DD/MM/YYYY hh:mm a'));
									_data.push(commentData.userDetails
										.displayName);
					...
					const loadPage = () => {
						...
						buildfire.publicData.search(searchOptions, 'posts',
							function (err, data) {
								if (data && data.result.length) {
									data.result.map(item => allPosts.push(
										item));
					...
					if (event) {
						switch (event.name) {
							...
							case EVENTS.POST_UNLIKED:
								ContentHome.posts.some(function (el) {
									if (el.id == event.id) {
										let index = el.likes.indexOf(event
											.userId)
							...
							case EVENTS.POST_DELETED:
								ContentHome.posts = ContentHome.posts.filter(function (
									el) {
				...
				window.onload = function () {
					buildfire.datastore.get("Social", function (err, result) {
						if (result.data.appSettings && result.data.appSettings
							.pinnedPost) {
							appSettings = result.data.appSettings;
							tinymce.activeEditor.setContent(result.data
								.appSettings.pinnedPost);
						}
					});
					setTimeout(() => {
